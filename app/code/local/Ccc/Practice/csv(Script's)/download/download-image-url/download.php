...
class DownloadFile
{
    ...
    protected $_destinationDir = 'image';
        ...
        if($handler)
        {
            while($row = fgetcsv($handler, 4096, ",", "\""))
            {
                ...
                else
                {
                    ...
                    {
                        $row["file_name"] = basename(urldecode($row["url"]));    
                    }
    ...
    protected function _downloadFile()
    {
        ...
        {    
            ...
            header("Refresh:1; url=http://cccserver.in/team/harish/scripts/download/download-image-url/download.php?s=$s");
        }
        else
        {
            unset($_SESSION['data']);
